...
pseudo.addEventListener('click', async () => {
    //http://example.org:8080/gpas/gpasService?wsdl
    //http://example.org:8080/gpas/DomainService?wsdl
    var url = 'http://localhost:8080/gpas/gpasService?wsdl';
    var args = {value: document.getElementById("vid").value, 
		domainName: 'NEMOPseudo'};
    await soap.createClient(url, function(err, client) {
        client.getOrCreatePseudonymFor(args, function(err, result) {
            alert(result.psn);
        });
    });
});
...
const patient_table = document.getElementById("pseudo-table-list").getElementsByTagName('TBODY')[0];
...
patient_table.addEventListener('click', (event) => {
    ...
    document.getElementById("vid").value = pid;
    document.getElementById("vfname").value = pfname;
    document.getElementById("vlname").value = plname;
    document.getElementById("vbdate").value = pbdate;
});
